...
public class VariantProductController {
    ...
    @GetMapping
    public ResponseEntity<List<VariantProductCardResponseDTO>> getAll() {
        log.info("⏳ Entrando al endpoint GET /api/v1/products/all");
        List<VariantProductCardResponseDTO> products = variantProductService.getAll();
        log.info("✅ Productos obtenidos correctamente: {}", products.size());
        return ResponseEntity.ok(products);
    }

    @GetMapping("/best-sellers")
    public ResponseEntity<List<VariantProductCardResponseDTO>> getBySold() {
        List<VariantProductCardResponseDTO> products = variantProductService.getBySold();
        return ResponseEntity.ok(products);
    }

    @GetMapping("/by-category/{id}")
    public ResponseEntity<List<VariantProductCardResponseDTO>> getByIdCategory(@PathVariable Long id) {
        List<VariantProductCardResponseDTO> products = variantProductService.getByIdCategory(id);
        return ResponseEntity.ok(products);
    }

    @GetMapping("/discount-by-category/{id}")
    public ResponseEntity<List<VariantProductCardResponseDTO>> getDiscountByIdCategory(@PathVariable Long id) {
        List<VariantProductCardResponseDTO> products = variantProductService.getDiscountByIdCategory(id);
        return ResponseEntity.ok(products);
    }

    @GetMapping("/filter")
    public ResponseEntity<Page<VariantProductCardResponseDTO>> getAllFilter(
            ...
            @RequestParam(defaultValue = "idVariantProductAsc") String sortBy) {
        String valAsc = "Asc";
        String valDesc = "Desc";
        String sortField = "";
        ...
        Page<VariantProductCardResponseDTO> products = variantProductService.getAllCardFilter(category, subcategory, brand, score,
                search,
    ...
    // public ResponseEntity<VariantProductCardResponseDTO> getProductDetailById(@PathVariable Long id) {
    //     log.info("GET /api/v1/products/{} - Retrieving product by ID", id);
    //     return variantProductService.getProductDetailById(id)
    ...
    public ResponseEntity<VariantProductCardResponseDTO> getById(@PathVariable Long id) {
        log.info("GET /api/v1/products/{} - Retrieving product by ID", id);
        return variantProductService.getById(id)
                .map(ResponseEntity::ok)
                .orElseThrow(() -> {
                    log.warn("Product with ID {} not found", id);
                    return new NotFoundException("Product with ID " + id + " not found");
